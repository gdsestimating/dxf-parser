		...
		while(!scanner.isEOF()) {
			if(curr.code === 0 && curr.value === 'SECTION') {
				...
				if (curr.value === 'HEADER') {
					...
					log.warn('INFO: Skipping section \'%s\'', curr.value);
				}
			} else {
				curr = scanner.next();
			}
...
function logUnhandledGroup(curr) {
	log.silly('unhandled group ' + debugCode(curr));
}
